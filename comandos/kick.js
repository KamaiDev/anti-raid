...
exports.run = async (client, message, args) => {
  ...
  if (!target) 
    return message.channel.send(a + ` | Este usuário não existe!`);
  if (!target.kickable)
  ...
  let confirm = new Discord.RichEmbed()
    ...
    .addField(
      `Reaja com ${b} para expulsar ou com ${a} para cancelar`,
  ...
  let embban = new Discord.RichEmbed()
    ...
    .setTimestamp(`• ${client.user.username} Kick System`);
  ...
  message.channel.send(confirm).then(c => {
    ...
    Cancelar.on("collect", r2 => {
      message.channel.send("Expulsão Cancelada...").then(mm => {
        mm.delete(3000);
      });
      c.delete();
    });
